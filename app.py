...
# =========================
# Constants & Writable Paths (Cloud-safe)
# =========================
PACK_SIZES = {"Hamburger": 8, "Hot Dog Buns": 10}   # units per pack
...
def _pick_writable_base() -> Path:
    ...
    - Else: ~/.cache/pepin as a fallback
    - Finally: current working dir (best effort)
    """
    candidates = [
        Path("/mount/data"),
...
BASE_DIR = (_pick_writable_base() / "pepin")
BASE_DIR.mkdir(parents=True, exist_ok=True)

PO_HISTORY_PATH = BASE_DIR / "po_history.csv"
WEEKLY_HISTORY_PATH = BASE_DIR / "weekly_sales_history.csv"

# ThriveMetrics → buns mapping (1 item = 1 bun unit). Add your variants as needed.
ITEM_TO_BUN = {
    "shack burger single": "Hamburger",
    "shack double burger": "Hamburger",
    "hot dog quarter pound": "Hot Dog Buns",
    # Examples of extra aliases you can add:
    # "burger single": "Hamburger",
    # "double burger": "Hamburger",
    # "quarter lb dog": "Hot Dog Buns",
}
...
def _read_csv_any(file):
    ...
    # Fallback if nothing looks like a header: try sniffer, start at 0
...
def _auto_detect_cols(df):
    ...
    if date_col and scores[date_col] < 0.4:
        date_col = None
    ...
    if qty_col and scores[qty_col] < 0.4:
        qty_col = None
...
def buns_weekly_from_thrives(df_items: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.dropna(subset=["bun_product"])  # keep only mapped items
    df["bun_units"] = df["qty"].astype(float)  # 1 row = bun_units (1:1 mapping)
...
if not prev_row.empty:
    ...
    we = pd.to_datetime(prev_row.iloc[0]["WeekEnd"]).dt.date()
...
# Stockout Risk Tue/Wed/Thu of delivery week
