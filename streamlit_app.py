...
st.set_page_config(page_title="Bread Inventory Dashboard (9-Day)", layout="wide")
st.title("🥖 Bread Inventory & PO Dashboard (9-Day, Deliveries @ 09:00)")

# --- Config ---
VARIANTS = ["French Baguette", "Hero", "Medianoche", "Multigrain"]

# Delivery days by index in 9-day horizon (Mon=0):
# Tue(1), Thu(3), Sat(5), Next Tue(8)
...
def norm_variant(x: str):
    if "baguette" in x: return "French Baguette"
    if "hero" in x: return "Hero"
    if "medianoche" in x: return "Medianoche"
    if "multigrain" in x: return "Multigrain"
    return None
